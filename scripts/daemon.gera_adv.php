...
while($res = mysql_fetch_array($sql)){
    ...
    for($i = 1;$i<=$qtdRegistros;$i++){
       // $j = ((3*$i)-3);
        ...
        $sql3 = mysql_query("SELECT id FROM faltas WHERE id_membro = '$id' AND status = 1 AND leitura = 0 LIMIT $j, 3") ;// or Logs(date("d-m-Y H:i:s")." arquivo de gerar_advertencia ".mysql_error(), 3); //die(mysql_error());
        ...
        if($haAdv == 0){
            if($quantidade == 3){
                $daAdv = mysql_query("INSERT INTO advertencia (id_membro, motivo, status, data) VALUES ('$id', 'Teve 3 faltas consecutivas', 0, '$data')") or die(mysql_error());
                if($daAdv){
                    //echo 'deu a adv';
                }else{
                    //echo 'eu odeio a adv';
